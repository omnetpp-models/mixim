...
class MIXIM_API PassedMessage : public cObject
{
    public:
        enum gates_t
        {
            UPPER_DATA, UPPER_CONTROL, LOWER_DATA, LOWER_CONTROL
        };
    ...
    public:
        ...
        PassedMessage() :
                cObject(), fromModule(-1), gateType(UPPER_CONTROL), direction(INCOMING), kind(0), name(NULL)
        {
        }
        PassedMessage(const PassedMessage& o) :
                cObject(o), fromModule(o.fromModule), gateType(o.gateType), direction(o.direction), kind(o.kind), name(
                        o.name)
        {
        }
