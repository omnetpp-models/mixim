...
BaseConnectionManager::BaseConnectionManager() :
        cSimpleModule(), nics(), coreDebug(false), sendDirect(false), playgroundSize(NULL), maxInterferenceDistance(
                0.0), maxDistSquared(0.0), useTorus(false), drawMIR(false), nicGrid(), findDistance(), gridDim()
{
}
...
void BaseConnectionManager::initialize(int stage)
{
    ...
    if (stage == 0)
    {
        ...
        { //fill the ColVector with copies of
            matrix.push_back(row); //the RowVector.
        }
        for (int i = 0; i < gridDim.x; ++i)
        { //fill the grid with copies of
...
BaseConnectionManager::GridCoord BaseConnectionManager::getCellForCoordinate(const Coord& c) const
{
    return GridCoord(c, findDistance);
}

void BaseConnectionManager::updateConnections(NicEntry::t_nicid_cref nicID, const Coord* oldPos, const Coord* newPos)
{
...
BaseConnectionManager::NicEntries& BaseConnectionManager::getCellEntries(const BaseConnectionManager::GridCoord& cell)
{
...
void BaseConnectionManager::checkGrid(BaseConnectionManager::GridCoord& oldCell,
        BaseConnectionManager::GridCoord& newCell, NicEntry::t_nicid_cref id)
...
void BaseConnectionManager::fillUnionWithNeighbors(CoordSet& gridUnion, const GridCoord& cell) const
{
...
bool BaseConnectionManager::isInRange(BaseConnectionManager::NicEntries::mapped_type pFromNic,
...
bool BaseConnectionManager::registerNic(cModule* nic, ConnectionManagerAccess* chAccess, const Coord* nicPos)
{
    assert(nic != NULL);
...
void BaseConnectionManager::updateNicPos(NicEntry::t_nicid_cref nicID, const Coord* newPos)
{
    ...
    if (ItNic == nics.end())
    {
        opp_warning("No nic with this ID (%d) is registered with this ConnectionManager, no position update done.",
                nicID);
        return;
    }
