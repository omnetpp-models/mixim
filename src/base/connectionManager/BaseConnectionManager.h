...
class MIXIM_API BaseConnectionManager : public cSimpleModule
{
        ...
        class GridCoord
        {
            ...
            public:
                ...
                GridCoord(const Coord& c, const Coord& gridCellSize = Coord(1.0, 1.0, 1.0)) :
                        x(static_cast<int>(c.x / gridCellSize.x)), y(static_cast<int>(c.y / gridCellSize.y)), z(
                                static_cast<int>(c.z / gridCellSize.z))
                {
                }
    ...
    private:
         ...
         */
        void checkGrid(GridCoord& oldCell, GridCoord& newCell, NicEntry::t_nicid_cref id);
