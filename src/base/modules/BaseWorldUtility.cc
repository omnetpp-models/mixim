...
BaseWorldUtility::BaseWorldUtility() :
        cSimpleModule(), playgroundSize(), useTorusFlag(false), use2DFlag(false), airFrameId(0), isInitialized(false)
{
}
...
void BaseWorldUtility::initializeIfNecessary()
{
    ...
    playgroundSize = Coord(par("playgroundSizeX").doubleValue(), par("playgroundSizeY").doubleValue(),
...
Coord BaseWorldUtility::getRandomPosition()
{
    ...
    return Coord(uniform(0, playgroundSize.x), uniform(0, playgroundSize.y),
