...
class TimeMappingIterator : public MappingIterator
{
    ...
    public:
        ...
        TimeMappingIterator(const iterator& it) :
                MappingIterator(), valueIt(it), position(), nextPosition(), isStepMapping(
                        it.getInterpolator().isStepping()), atPreStep(false)
        {
        ...
        TimeMappingIterator(const TimeMappingIterator<Interpolator>& o) :
                MappingIterator(o), valueIt(o.valueIt), position(o.position), nextPosition(o.nextPosition), isStepMapping(
                        o.isStepMapping), atPreStep(o.atPreStep)
        {
        }
...
class MIXIM_API LinearIntplMappingIterator : public MappingIterator
{
    ...
    public:
         ...
         */
        LinearIntplMappingIterator(ConstMappingIterator* leftIt, ConstMappingIterator* rightIt,
                argument_value_cref_t f);
...
class MIXIM_API LinearIntplMapping : public Mapping
{
    ...
    public:
         ...
         */
        LinearIntplMapping(const ConstMapping * const left = NULL, const ConstMapping * const right = NULL,
        ...
        virtual MappingIterator* createIterator(const Argument& pos)
        {
            ...
            return new LinearIntplMappingIterator(left->createConstIterator(pos), right->createConstIterator(pos),
                    factor);
        }
         ...
         */
        virtual void setValue(const Argument&, argument_value_cref_t)
        {
            assert(false);
        }
};
 ...
 */
template<>
class Linear<std::map<Argument::mapped_type, Mapping*> > : public InterpolatorBase<
        std::map<Argument::mapped_type, Mapping*> >
{
    protected:
    ...
    public:
         ...
         * "interpolated".
         */
        virtual interpolated operator()(const const_iterator& first, const const_iterator& last, key_cref_type pos,
                const_iterator upperBound) const
        {
            ...
            return interpolated(
                    LinearIntplMapping(left->second, right->second,
                            linearInterpolationFactor(pos, left->first, right->first)));
        }
    ...
    protected:
         ...
         * LinearIntplMappings.
         */
        static ConstMapping::argument_value_t linearInterpolationFactor(key_cref_type t, key_cref_type t0,
                key_cref_type t1)
        {
...
class MultiDimMappingIterator : public MappingIterator
{
    ...
    public:
        ...
        MultiDimMappingIterator(MultiDimMapping<Interpolator>& pMapping) :
                MappingIterator(), mapping(pMapping), valueIt(pMapping.entries.beginIntpl()), subMapping(0), subIterator(
                        NULL), position(), nextPosition()
        {
        ...
        MultiDimMappingIterator(MultiDimMapping<Interpolator>& pMapping, const Argument& pos) :
                MappingIterator(), mapping(pMapping), valueIt(
                        pMapping.entries.beginIntpl()/*pMapping.entries.findIntpl(pos.getArgValue(pMapping.myDimension))*/), //ATTENTION: pMapping.entries.findIntpl(...) results in GCC-Crash at -O2
                subMapping(0), subIterator(NULL), position(pos), nextPosition()
        {
        ...
        MultiDimMappingIterator(const MultiDimMappingIterator& o) :
                MappingIterator(o), mapping(o.mapping), valueIt(o.valueIt), subMapping(o.subMapping), subIterator(NULL), position(
                        o.position), nextPosition(o.nextPosition)
        {
            // valueIt was not initialized with pMapping.entries.findIntpl(...), so we need the jumpTo-call
            if (*subMapping)
            {
...
class MultiDimMapping : public Mapping
{
    ...
    protected:
        ...
        MultiDimMapping(const DimensionSet& myDims, Dimension myDim) :
                Mapping(myDims), outOfRangeMapping(0), wrappedOORMapping(0), entries(), myDimension(myDim), isMaster(
                        false)
        {
        }
         ...
         * This constructor is only used internally to create the sub-mappings.
         */
        MultiDimMapping(const DimensionSet& myDims, Dimension myDim, ConstantSimpleConstMapping* oorm,
                ConstMappingWrapper* wrappedoorm) :
                Mapping(myDims), outOfRangeMapping(oorm), wrappedOORMapping(wrappedoorm), entries(wrappedOORMapping), myDimension(
         ...
         */
        MultiDimMapping(const MultiDimMapping<Interpolator>& o, ConstantSimpleConstMapping* oorm,
                ConstMappingWrapper* wrappedoorm) :
                Mapping(o), outOfRangeMapping(oorm), wrappedOORMapping(wrappedoorm), entries(o.entries), myDimension(
                        o.myDimension), isMaster(false)
        {
        ...
        void copySubMappings()
        {
            ...
            else
            {
                for (typename interpolator_map_type::iterator it = entries.begin(); it != itEnd; ++it)
                {
                    ...
                    {
                        it->second = new MultiDimMapping<Interpolator>(
                                *(static_cast<MultiDimMapping<Interpolator>*>(it->second)));
                    }
                    else
                    {
                        it->second = new MultiDimMapping<Interpolator>(
                                *(static_cast<MultiDimMapping<Interpolator>*>(it->second)), outOfRangeMapping,
                                wrappedOORMapping);
                    }
                }
            }
    ...
    public:
        ...
        MultiDimMapping(const DimensionSet& myDims) :
                Mapping(myDims), outOfRangeMapping(0), wrappedOORMapping(0), entries(), myDimension(), isMaster(true)
        {
        ...
        MultiDimMapping(const DimensionSet& myDims, argument_value_cref_t oorv) :
                Mapping(myDims), outOfRangeMapping(new ConstantSimpleConstMapping(myDims, oorv)), wrappedOORMapping(
                        new ConstMappingWrapper(outOfRangeMapping)), entries(wrappedOORMapping), myDimension(), isMaster(
                        true)
        {
        ...
        MultiDimMapping(const MultiDimMapping<Interpolator>& o) :
                Mapping(o), outOfRangeMapping(o.outOfRangeMapping), wrappedOORMapping(o.wrappedOORMapping), entries(
                        o.entries), myDimension(o.myDimension), isMaster(true)
        {
...
class MIXIM_API MappingUtils
{
    ...
    public:
         ...
         * Note: The interpolation method is always linear, at the moment.
         */
        static Mapping* createMapping(Mapping::argument_value_cref_t outOfRangeValue, const DimensionSet& domain =
                DimensionSet(Dimension::time), Mapping::InterpolationMethod intpl = Mapping::LINEAR);
        ...
        static Mapping* applyElementWiseOperator(const ConstMapping& f1, const ConstMapping& f2, Operator op,
                Mapping::argument_value_cref_t outOfRangeVal = Argument::MappedZero, bool contOutOfRange = true)
        {
            ...
            Mapping * const result =
                    (contOutOfRange) ? MappingUtils::createMapping(domain) :
                            MappingUtils::createMapping(outOfRangeVal, domain);
         ...
         */
        static Argument::mapped_type findMax(const ConstMapping& m, Argument::mapped_type_cref cRetNotFound =
                cMaxNotFound);
         ...
         */
        static Argument::mapped_type findMax(const ConstMapping& m, const Argument& min, const Argument& max,
         ...
         */
        static Argument::mapped_type findMin(const ConstMapping& m, Argument::mapped_type_cref cRetNotFound =
                cMinNotFound);
         ...
         * @return The value at the key entry with the highest value or <tt>cRetNotFound</tt> if map is empty or no element in range [min,max].
         */
        static Argument::mapped_type findMin(const ConstMapping& m, const Argument& min, const Argument& max,
         ...
         *
         */
        static void addDiscontinuity(Mapping* m, const Argument& pos, Argument::mapped_type_cref value,
...
//--------members----------
    ...
    public:
        ConcatConstMappingIterator(ConstMapping* baseMapping) :
                FilteredConstMappingIterator(baseMapping->createConstIterator()), baseMapping(baseMapping)
        {
        }

        ConcatConstMappingIterator(ConstMapping* baseMapping, const Argument& pos) :
                FilteredConstMappingIterator(baseMapping->createConstIterator(pos)), baseMapping(baseMapping)
        {
        }
...
class ConcatConstMapping : public ConstMapping
{
    ...
    public:
        ConcatConstMapping(const ConcatConstMapping& o) :
                ConstMapping(o), mappings(o.mappings), refMapping(NULL), continueOutOfRange(o.continueOutOfRange), oorValue(
                        o.oorValue), op()
        {
        }
    ...
    public:
        ...
        ConcatConstMapping(ConstMapping* refMapping, Iterator first, Iterator last, bool continueOutOfRange = true,
                Argument::mapped_type_cref oorValue = Argument::MappedZero, Operator op = Operator()) :
                ConstMapping(refMapping->getDimensionSet()), mappings(), refMapping(refMapping), continueOutOfRange(
         ...
         */
        ConcatConstMapping(ConstMapping* refMapping, ConstMapping* other, bool continueOutOfRange = true,
                Argument::mapped_type_cref oorValue = Argument::MappedZero, Operator op = Operator()) :
                ConstMapping(refMapping->getDimensionSet()), mappings(), refMapping(refMapping), continueOutOfRange(
                        continueOutOfRange), oorValue(oorValue), op(op)
        {
        ...
        Mapping* createConcatenatedMapping() const
        {
            ...
            Mapping* result = MappingUtils::applyElementWiseOperator(*refMapping, **it, op, oorValue,
                    continueOutOfRange);

            while (++it != itEnd)
            {
                Mapping* buf = result;
                result = MappingUtils::applyElementWiseOperator(*buf, **it, op, oorValue, continueOutOfRange);
                delete buf;
            }
