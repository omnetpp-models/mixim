...
inline Argument::iterator Argument::insertValue(iterator pos, const Argument::value_type& valPair, iterator& itEnd,
        bool ignoreUnknown)
{
...
bool Argument::isClose(const Argument& o, Argument::mapped_type_cref epsilon) const
{
    ...
    for (const_iterator it = begin(); it != itEnd && itO != itEndO; ++it)
    {
...
SimpleConstMappingIterator::SimpleConstMappingIterator(const ConstMapping* mapping,
        ...
        ConstMappingIterator(), mapping(mapping), dimensions(mapping->getDimensionSet()), position(
                mapping->getDimensionSet(), start.getTime()), keyEntries(keyEntries), nextEntry()
{
    assert(keyEntries);
...
SimpleConstMappingIterator::SimpleConstMappingIterator(const ConstMapping* mapping,
        ...
        ConstMappingIterator(), mapping(mapping), dimensions(mapping->getDimensionSet()), position(dimensions), keyEntries(
                keyEntries), nextEntry()
{
    assert(keyEntries);
...
void SimpleConstMapping::initializeArguments(const Argument& min, const Argument& max, const Argument& interval)
{
    keyEntries.clear();
