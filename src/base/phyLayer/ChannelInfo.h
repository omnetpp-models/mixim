...
class MIXIM_API ChannelInfo
{
    ...
    protected:
         ...
         */
        template<typename C, typename ItMatrix = typename C::const_iterator,
                typename ItSubMatrix = typename C::mapped_type::const_iterator>
        class BaseIntersectionIterator
        {
            ...
            public:
                ...
                BaseIntersectionIterator(const iterator& __it) :
                        intervals(__it.intervals), from(__it.from), to(__it.to), endIt(__it.endIt), startIt(
                                __it.startIt), startItEnd(__it.startItEnd)
                {
                }
                ...
                bool operator==(const _Self& __x) const
                {
                    ...
                    if ((endIt == intervals->end() && __x.endIt == __x.intervals->end())
                            || (startIt == startItEnd && __x.startIt == __x.startItEnd))
                        return true;
                    return intervals == __x.intervals && from == __x.from && to == __x.to && endIt == __x.endIt
                            && startIt == __x.startIt && startItEnd == __x.startItEnd;
                }
        ...
        typedef BaseIntersectionIterator<const AirFrameMatrix, AirFrameMatrix::const_iterator,
        ...
        typedef BaseIntersectionIterator<AirFrameMatrix, AirFrameMatrix::iterator, AirFrameMatrix::mapped_type::iterator> iterator;
    ...
    protected:
         ...
         * passed as parameter.
         */
        static void getIntersections(const AirFrameMatrix& airFrames, simtime_t_cref from, simtime_t_cref to,
         ...
         */
        static bool isIntersecting(const AirFrameMatrix& airFrames, simtime_t_cref from, simtime_t_cref to);
         ...
         */
        static void deleteAirFrame(AirFrameMatrix& airFrames, airframe_ptr_t a, simtime_t_cref startTime,
    ...
    public:
        ChannelInfo() :
                activeAirFrames(), inactiveAirFrames(), airFrameStarts(), recordStartTime(-1)
        {
        }
