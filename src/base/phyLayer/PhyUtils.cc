...
Radio::Radio(int numRadioStates, bool recordStats, int initialState, Argument::mapped_type_cref minAtt,
        ...
        radioStates(), radioChannels(), state(initialState), nextState(initialState), numRadioStates(numRadioStates), swTimes(
                ...
                currentChannel), nbChannels(nbChannels)
{
    assert(nbChannels > 0);
...
RSAMConstMappingIterator::RSAMConstMappingIterator(const RadioStateAnalogueModel* rsam, simtime_t_cref signalStart,
        simtime_t_cref signalEnd) :
        ConstMappingIterator(), rsam(rsam), it(), position(), nextPosition(), signalStart(signalStart), signalEnd(
                signalEnd)
{
    assert(rsam);
...
bool RSAMConstMappingIterator::inRange() const
{
    ...
    return signalStart <= t && t <= signalEnd && t <= lastEntryTime;
...
bool RSAMConstMappingIterator::hasNext() const
{
    ...
    return position.getTime() < signalStart || (it2 != rsam->radioStateAttenuation.end() && it2->getTime() <= signalEnd);
}
