...
class MIXIM_API BaseDecider : public Decider
{
    ...
    protected:
        ...
        typedef struct tCSRInfo
        {
                ...
                tCSRInfo() :
                        first(NULL), second(), canAnswerAt()
                {
                }
                tCSRInfo(const tCSRInfo& o) :
                        first(o.first), second(o.second), canAnswerAt(o.canAnswerAt)
                {
                }
                ...
                ChannelSenseRequest* getRequest() const
                {
                    return first;
                }
                void setRequest(ChannelSenseRequest* request)
                {
                    first = request;
                }
                simtime_t_cref getSenseStart() const
                {
                    return second;
                }
                void setSenseStart(simtime_t_cref start)
                {
                    second = start;
                }
                simtime_t_cref getAnswerTime() const
                {
                    return canAnswerAt;
                }
                void setAnswerTime(simtime_t_cref answerAt)
                {
                    canAnswerAt = answerAt;
                }
        } CSRInfo;
    ...
    public:
        ...
        BaseDecider(DeciderToPhyInterface* phy, double sensitivity, int myIndex, bool debug) :
                Decider(phy), sensitivity(sensitivity), currentSignal(), isChannelIdle(true), currentChannelSenseRequest(), myIndex(
                        myIndex), debug(debug)
        {
