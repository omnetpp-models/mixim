...
class MIXIM_API Argument
{
    ...
    protected:
         ...
         */
        inline iterator insertValue(iterator pos, const Argument::value_type& valPair, iterator& itEnd,
...
class MIXIM_API ConstMapping
{
    ...
    public:
        ...
        stream& print(stream& out, argument_value_cref_t lTimeScale = argument_value_t(1000),
                ...
                const std::string& sTableHead = std::string("o\\ms"), const Dimension * const pOnlyDim = NULL) const
        {
            ...
            if (bIs2Dim || bOnlyDimFound)
            {
                ...
                if (bOnlyDimFound && (posValIt = it->getPosition().find(otherDim)) != it->getPosition().end())
                {
                    ...
                    iMaxLeftItemLen = std::max(iMaxLeftItemLen,
                            m.toString(posValIt->second * lLeftColScale, 2).length());
                }
                else if (!bOnlyDimFound)
                {
                    otherPositions.insert(it->getPosition().begin()->second);
                    iMaxLeftItemLen = std::max(iMaxLeftItemLen,
                            m.toString(it->getPosition().begin()->second * lLeftColScale, 2).length());
                }
            }
                ...
                for (Argument::const_iterator itA = pos.begin(); itA != posValItEnd; ++itA)
                {
                    if (bIs2Dim || bOnlyDimFound)
                    {
                        if (!bOnlyDimFound || itA->first == otherDim)
                        {
                            ...
                            iMaxLeftItemLen = std::max(iMaxLeftItemLen,
                                    m.toString(itA->second * lLeftColScale, 2).length());
                        }
                    }
            ...
            {
                out << "Defines no own key entries for " << pOnlyDim->getName()
            ...
            osBorder.fill('-');
            osBorder << std::setw(iMaxLeftItemLen) << "" << osBorder.fill() << "+" << osBorder.fill()
                    << std::setw(osHeader.str().length()) << "";
            ...
            else
            {
                ...
                while (it->inRange())
                {
                    ...
                    {
                        ++fIt;
                        out << std::endl << m.toString((*fIt) * lLeftColScale, iMaxLeftItemLen) << out.fill() << "|"
                                << out.fill();
                        tIt = timePositions.begin();
...
class MIXIM_API SimpleConstMappingIterator : public ConstMappingIterator
{
    ...
    public:
        ...
        SimpleConstMappingIterator(const ConstMapping* mapping,
                const SimpleConstMappingIterator::KeyEntrySet* keyEntries, const Argument& start);
        ...
        {
            return !(keyEntries->empty() || (*(keyEntries->rbegin()) < position) || (position < *(keyEntries->begin())));
        }
