...
class MIXIM_API ProbabilisticBroadcast : public BaseNetwLayer
{
    ...
    public:
        ProbabilisticBroadcast() :
                BaseNetwLayer(), broadcastPeriod(), beta(0), timeToLive(), maxNbBcast(0), maxFirstBcastBackoff(0), timeInQueueAfterDeath(
                        0), headerLength(0), trace(false), stats(false), debug(false), broadcastTimer(NULL), knownMsgIds(), msgQueue(), debugMsgIdSet(), nbDataPacketsReceived(
                        0), nbDataPacketsSent(0), nbHops(0), debugNbMessageKnown(0), nbDataPacketsForwarded(0), oneHopLatencies()
        {
        }
    ...
    protected:
        enum messagesTypes
        {
            UNKNOWN = 0, BROADCAST_TIMER, NEIGHBOR_TIMER, BETA_TIMER, DATA
        };
