...
void ProbabilisticBroadcast::handleUpperMsg(cMessage* msg)
{
    ...
    nbDataPacketsSent++;
    EV << "PBr: " << simTime() << " n" << myNetwAddr << " handleUpperMsg(): Pkt ID = " << pkt->getId() << " TTL = "
            << pkt->getAppTtl() << endl;
...
void ProbabilisticBroadcast::handleLowerMsg(cMessage* msg)
{
    ...
    if (/*(m->getAppTtl() <= 0) || */(messageKnown(m->getId())))
    {
        ...
        EV << "PBr: " << simTime() << " n" << myNetwAddr << " handleLowerMsg(): Dead or Known message ID=" << m->getId()
                << " from node " << macSrcAddr << " TTL = " << m->getAppTtl() << endl;
        delete m;
    }
    else
    {
        ...
        {
            ++debugNbMessageKnown;
            EV << "PBr: " << simTime() << " n" << myNetwAddr << " ERROR Message should be known TTL= " << m->getAppTtl()
                    << endl;
        }
        EV << "PBr: " << simTime() << " n" << myNetwAddr << " handleLowerMsg(): Unknown message ID=" << m->getId()
...
void ProbabilisticBroadcast::handleSelfMsg(cMessage* msg)
{
    if (msg == broadcastTimer)
    {
        ...
        EV << "PBr: " << simTime() << " n" << myNetwAddr << " handleSelfMsg(): Message ID= " << pkt->getId() << " TTL= "
                << pkt->getAppTtl() << endl;
        if (pkt->getAppTtl() > 0)
        {
...
{
    EV << "PBr: " << simTime() << " n" << myNetwAddr << " Received lower control message. Name: " << msg->getName()
            << " type: " << msg->getKind() << endl;
    delete msg;
}
...
void ProbabilisticBroadcast::insertMessage(simtime_t_cref bcastDelay, tMsgDesc* msgDesc)
{
    ...
    EV << "PBr: " << simTime() << " n" << myNetwAddr << "         insertMessage() bcastDelay = " << bcastDelay
    ...
    {
        EV << "PBr: " << simTime() << " n" << myNetwAddr << "         message inserted in the front, reschedule it."
                << endl;
...
ProbabilisticBroadcast::tMsgDesc* ProbabilisticBroadcast::popFirstMessageUpdateQueue(void)
{
    ...
    EV << "PBr: " << simTime() << " n" << myNetwAddr << "         pop(): just popped msg " << msgDesc->pkt->getId()
            << endl;
    if (!msgQueue.empty())
    {
...
void ProbabilisticBroadcast::insertNewMessage(ProbabilisticBroadcastPkt* pkt, bool iAmInitialSender)
{
    ...
    if (ttl > 0)
    {
        ...
        if (bcastDelay > ttl)
            bcastDelay = ttl;
        EV << "PBr: " << simTime() << " n" << myNetwAddr << " insertNewMessage(): insert packet " << pkt->getId()
                << " with delay " << bcastDelay << endl;
