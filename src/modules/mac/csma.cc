...
void csma::initialize(int stage)
{
        ...
        if (phyModule->hasPar("timeRXToTX"))
        {
            ...
            if (rxToTx > aTurnaroundTime)
            {
                opp_warning("Parameter \"aTurnaroundTime\" (%f) does not match"
                        ...
                        " should be equal or bigger", SIMTIME_DBL(aTurnaroundTime), SIMTIME_DBL(rxToTx));
            }
        }
    ...
    else if (stage == 1)
    {
        ...
        debugEV
                << "queueLength = " << queueLength << " bitrate = " << bitrate << " backoff method = "
                        << par("backoffMethod").stringValue() << endl;
...
void csma::handleUpperMsg(cMessage *msg)
{
    ...
    debugEV
            << "CSMA received a message from upper layer, name is " << msg->getName() << ", CInfo removed, mac addr="
                    << getUpperDestinationFromControlInfo(cInfo) << endl;
...
void csma::updateStatusBackoff(t_mac_event event, cMessage *msg)
{
    switch (event)
    {
        case EV_TIMER_BACKOFF:
            debugEV << "(2) FSM State BACKOFF, EV_TIMER_BACKOFF:" << " starting CCA timer." << endl;
    ...
    switch (event)
    {
        case EV_TIMER_CCA: {
            ...
            else
            {
                ...
                debugEV << "(7) FSM State CCA_3, EV_TIMER_CCA, [Channel Busy]: " << " increment counters." << endl;
                NB = NB + 1;
                ...
                if (NB > macMaxCSMABackoffs)
                {
                    // drop the frame
                    debugEV
                            << "Tried " << NB << " backoffs, all reported a busy " << "channel. Dropping the packet."
                                    << endl;
        ...
        case EV_FRAME_RECEIVED:
            ...
            break;
        case EV_BROADCAST_RECEIVED:
            debugEV << "(24) FSM State BACKOFF, EV_BROADCAST_RECEIVED:" << " Nothing to do." << endl;
            sendUp(decapsMsg(static_cast<MacPkt *>(msg)));
            delete msg;
            break;
        default:
...
void csma::updateStatusTransmitFrame(t_mac_event event, cMessage *msg)
{
    if (event == EV_FRAME_TRANSMITTED)
    {
        ...
        {
            //unicast
            debugEV << "(4) FSM State TRANSMITFRAME_4, " << "EV_FRAME_TRANSMITTED [Unicast]: ";
        }
        else
        {
            //broadcast
            debugEV << "(27) FSM State TRANSMITFRAME_4, EV_FRAME_TRANSMITTED " << " [Broadcast]";
            expectAck = false;
        }
...
void csma::updateStatusWaitAck(t_mac_event event, cMessage *msg)
{
    ...
    switch (event)
    {
        case EV_ACK_RECEIVED:
            debugEV << "(5) FSM State WAITACK_5, EV_ACK_RECEIVED: " << " ProcessAck, manageQueue..." << endl;
        ...
        case EV_ACK_TIMEOUT:
            debugEV
                    << "(12) FSM State WAITACK_5, EV_ACK_TIMEOUT:" << " incrementCounter/dropPacket, manageQueue..."
                            << endl;
...
void csma::manageMissingAck(t_mac_event /*event*/, cMessage */*msg*/)
{
    if (txAttempts < macMaxFrameRetries + 1)
    {
        // increment counter
        txAttempts++;
        debugEV << "I will retransmit this packet (I already tried " << txAttempts << " times)." << endl;
    }
    else
    {
        // drop packet
        debugEV
                << "Packet was transmitted " << txAttempts << " times and I never got an Ack. I drop the packet."
                        << endl;
        cMessage * mac = macQueue.front();
        macQueue.pop_front();
        txAttempts = 0;
...
void csma::updateStatusSIFS(t_mac_event event, cMessage *msg)
{
    ...
    {
        case EV_TIMER_SIFS:
            debugEV << "(17) FSM State WAITSIFS_6, EV_TIMER_SIFS:" << " sendAck -> TRANSMITACK." << endl;
        ...
        case EV_TIMER_BACKOFF:
            ...
            // and stay here.
            debugEV
                    << "(16) FSM State WAITSIFS_6, EV_TIMER_BACKOFF. " << "Restart backoff timer and don't move."
                            << endl;
            startTimer(TIMER_BACKOFF);
            break;
        case EV_BROADCAST_RECEIVED:
...
void csma::updateStatusTransmitAck(t_mac_event event, cMessage *msg)
{
    ...
    if (event == EV_FRAME_TRANSMITTED)
    {
        debugEV << "(19) FSM State TRANSMITACK_7, EV_FRAME_TRANSMITTED:" << " ->manageQueue." << endl;
        phy->setRadioState(Radio::RX);
        //		delete msg;
        manageQueue();
    }
...
void csma::updateStatusNotIdle(cMessage *msg)
{
    ...
    if (macQueue.size() <= queueLength)
    {
        macQueue.push_back(static_cast<MacPkt *>(msg));
        debugEV
                << "(21) FSM State NOT IDLE, EV_SEND_REQUEST" << " and [TxBuff avail]: enqueue packet and don't move."
                        << endl;
    }
    else
    {
        // queue is full, message has to be deleted
        debugEV
                << "(22) FSM State NOT IDLE, EV_SEND_REQUEST"
...
void csma::manageQueue()
{
    ...
    {
        debugEV
                << "(manageQueue) there are " << macQueue.size() << " packets to send, entering backoff wait state."
                        << endl;
...
void csma::startTimer(t_mac_timer timer)
{
    ...
    else if (timer == TIMER_CCA)
    {
        ...
        debugEV
                << "(startTimer) ccaTimer value=" << ccaTime << "(rxSetupTime,ccaDetectionTime:" << rxSetupTime << ","
...
simtime_t csma::scheduleBackoff()
{
    ...
    switch (backoffMethod)
    {
        case EXPONENTIAL: {
            ...
            debugEV
                    << "(startTimer) backoffTimer value=" << backoffTime << " (BE=" << BE << ", 2^BE-1= " << v << "r="
                            << r << ")" << endl;
            break;
        }
        case LINEAR: {
...
void csma::handleLowerMsg(cMessage *msg)
{
    ...
    debugEV
            << "Received frame name= " << macPkt->getName() << ", myState=" << macState << " src="
                    << macPkt->getSrcAddr() << " dst=" << macPkt->getDestAddr() << " myAddr=" << myMacAddr << endl;
...
//			nbRxFrames++;
        ...
        else
        {
            ...
            if (strcmp(macPkt->getName(), "CSMA-Ack") != 0)
            {
                ...
                debugEV << "Received a data packet addressed to me," << " preparing an ack..." << endl;
...
//				nbRxFrames++;
                ...
                else
                {
                    ...
                    debugEV
                            << "Expected Sequence number is " << ExpectedNr << " and number of packet is " << SeqNr
                                    << endl;
                    if (SeqNr < ExpectedNr)
                    {
            ...
            else if (macQueue.size() != 0)
            {
                ...
                {
                    EV << "Error! Received an ack from an unexpected source: src=" << macPkt->getSrcAddr()
                            << ", I was expecting from node addr=" << firstPacket->getDestAddr() << endl;
                    delete macPkt;
                }
            }
            else
            {
                EV << "Error! Received an Ack while my send queue was empty. src=" << macPkt->getSrcAddr() << "."
                        << endl;
                delete macPkt;
            }
        }
...
void csma::handleLowerControl(cMessage *msg)
{
    ...
    {
        EV << "Invalid control message type (type=NOTHING) : name=" << msg->getName() << " modulesrc="
