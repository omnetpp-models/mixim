...
void CSMAMacLayer::initialize(int stage)
{
    ...
    else if (stage == 1)
    {
        ...
        debugEV
                ...
                //<< " busyRSSI = " << busyRSSI
                        << " slotDuration = " << slotDuration << " difs = " << difs << " maxTxAttempts = "
                        << maxTxAttempts << " bitrate = " << bitrate << " contentionWindow = " << initialCW << endl;
...
CSMAMacLayer::~CSMAMacLayer()
{
    ...
    for (MacQueue::iterator it = macQueue.begin(); it != macQueue.end(); ++it)
    {
        delete (*it);
    }
    macQueue.clear();
...
void CSMAMacLayer::handleUpperMsg(cMessage *msg)
{
    ...
    if (macQueue.size() <= queueLength)
    {
        ...
        debugEV
                << "packet putt in queue\n  queue size:" << macQueue.size() << " macState:" << macState << " (RX=" << RX
                        << ") is scheduled:" << backoffTimer->isScheduled() << endl;
        ;
...
void CSMAMacLayer::scheduleBackoff()
{
    if (backoffTimer->isScheduled())
    {
        std::cerr << " is scheduled: MAC state " << macState << " radio state : " << phy->getRadioState() << endl;
    }
...
MacPkt* CSMAMacLayer::encapsMsg(cPacket *pkt)
{
    ...
    {
        EV << "Warning: Sending packet " << pkt << " - duration (" << duration
                << " ) is bigger than the slot duration (" << slotDuration << ")." << endl;
    }
