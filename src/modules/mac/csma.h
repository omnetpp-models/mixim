...
class MIXIM_API csma : public BaseMacLayer
{
    public:
        csma() :
                BaseMacLayer(), nbTxFrames(0), nbRxFrames(0), nbMissedAcks(0), nbRecvdAcks(0), nbDroppedFrames(0), nbTxAcks(
                        0), nbDuplicates(0), nbBackoffs(0), backoffValues(0), stats(false), trace(false), backoffTimer(
                        NULL), ccaTimer(NULL), sifsTimer(NULL), rxAckTimer(NULL), macState(IDLE_1), status(STATUS_OK), sifs(), macAckWaitDuration(), transmissionAttemptInterruptedByRx(
                        false), ccaDetectionTime(), rxSetupTime(), aTurnaroundTime(), macMaxCSMABackoffs(0), macMaxFrameRetries(
                        0), aUnitBackoffPeriod(), useMACAcks(false), backoffMethod(CONSTANT), macMinBE(0), macMaxBE(0), initialCW(
                        0), txPower(0), NB(0), macQueue(), queueLength(0), txAttempts(0), bitrate(0), droppedPacket(), nicId(
                        -1), ackLength(0), ackMessage(NULL), SeqNrParent(), SeqNrChild()
        {
        }
