...
void BMacLayer::handleSelfMsg(cMessage *msg)
{
    switch (macState)
    {
        ...
        case CCA:
            if (msg->getKind() == BMAC_CCA_TIMEOUT)
            {
                ...
                {
                    debugEV << "State CCA, message CCA_TIMEOUT, new state SLEEP" << endl;
            ...
            {
                nbRxDataPackets++;
                debugEV << "State CCA, message BMAC_DATA, new state WAIT_DATA" << endl;
                macState = WAIT_DATA;
                cancelEvent(cca_timeout);
                scheduleAt(simTime() + slotDuration + checkInterval, data_timeout);
        ...
        case WAIT_ACK:
            ...
            if (msg->getKind() == BMAC_ACK)
            {
                ...
                debugEV
                        << "We are waiting for ACK from : " << lastDataPktDestAddr << ", and ACK came from : " << src
                                << endl;
        ...
        case WAIT_DATA:
            ...
            {
                //nothing happens
                debugEV << "State WAIT_DATA, message BMAC_ACK, new state WAIT_DATA" << endl;
                delete msg;
                return;
            }
            if (msg->getKind() == BMAC_DATA)
            {
                ...
                {
                    debugEV << "State WAIT_DATA, message BMAC_DATA, new state SLEEP" << endl;
                    // if something in the queue, wakeup soon.
                    if (macQueue.size() > 0)
                        scheduleAt(simTime() + dblrand() * checkInterval, wakeup);
                    else
    ...
    opp_error("Undefined event of type %d in state %d (Radio state %d)!", msg->getKind(), macState,
            phy->getRadioState());
}
