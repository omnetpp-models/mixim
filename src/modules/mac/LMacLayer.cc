...
void LMacLayer::initialize(int stage)
{
    ...
    else if (stage == 1)
    {
        ...
        debugEV
                << "queueLength = " << queueLength << " slotDuration = " << slotDuration << " controlDuration = "
                        << controlDuration << " numSlots = " << numSlots << " bitrate = " << bitrate << endl;
    ...
    switch (macState)
    {
        case INIT:
            if (msg->getKind() == LMAC_START_LMAC)
            {
                ...
                debugEV
                        << "ID: " << FindModule<>::findHost(this)->getId() << ". Picked random slot: " << mySlot
                                << endl;
        ...
        case SLEEP:
            if (msg->getKind() == LMAC_WAKEUP)
            {
                ...
                if (SETUP_PHASE)
                {
                    ...
                    debugEV
                            << "setup phase slot duration:" << 2.f * controlDuration << "while controlduration is"
                                    << controlDuration << endl;
                }
                else
        ...
        case CCA:
            ...
            else if (msg->getKind() == LMAC_CONTROL)
            {
                ...
                debugEV
                        << " I have received a control packet from src " << mac->getSrcAddr() << " and dest " << dest
                                << ".\n";
                ...
                if (((mySlot > -1) && (mac->getOccupiedSlots(mySlot) > LMAC_FREE_SLOT)
                        && (mac->getOccupiedSlots(mySlot) != myMacAddr)) || collision)
                {
