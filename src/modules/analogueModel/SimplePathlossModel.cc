...
SimplePathlossConstMapping::SimplePathlossConstMapping(const DimensionSet& dimensions, SimplePathlossModel* model,
        const double distFactor) :
        SimpleConstMapping(dimensions), distFactor(distFactor), model(model), hasFrequency(
                dimensions.hasDimension(Dimension::frequency))
{
}

SimplePathlossConstMapping::SimplePathlossConstMapping(const SimplePathlossConstMapping& o) :
        SimpleConstMapping(o), distFactor(o.distFactor), model(o.model), hasFrequency(o.hasFrequency)
{
}
...
void SimplePathlossModel::filterSignal(MiximAirFrame *frame, const Coord& sendersPos, const Coord& receiverPos)
{
    Signal& signal = frame->getSignal();

    /** Calculate the distance factor */
    double sqrDistance =
            useTorus ? receiverPos.sqrTorusDist(sendersPos, playgroundSize) : receiverPos.sqrdist(sendersPos);
