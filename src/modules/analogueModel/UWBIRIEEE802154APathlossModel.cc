...
const bool UWBIRIEEE802154APathlossModel::implemented_CMs[] = { false, //  There is no Channel Model 0: start at 1
        true, //  CM1
        true, //  CM2
        true, //  CM3
        ...
        const Coord& /*receiverPos*/)
{
    ...
    // create mapping
    SimpleTimeConstMapping* attMapping = new SimpleTimeConstMapping(attenuation, signal.getReceptionStart(),
            signal.getReceptionEnd());
        ...
        while (moreTaps)
        {
            ...
            while (currentPoint < echoEnd)
            {
                ...
                else if (!raising && pulsesIter->getNextPosition().getTime() < echoEnd)
                {
                    ...
                    // interpolate current echo point
                    double echoValue = 1
                            - SIMTIME_DBL((currentPoint - echoPeak) / (0.5 * IEEE802154A::mandatory_pulse));
                    echoValue = echoValue * finalTapEnergy;
                    intermediatePoints[currentPoint] = echoValue + oldValue;
                }
                else if (!raising && pulsesIter->getNextPosition().getTime() >= echoEnd)
