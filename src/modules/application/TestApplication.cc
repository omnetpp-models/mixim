...
void TestApplication::handleMessage(cMessage * msg)
{
    ...
    else if (msg->getArrivalGateId() == dataIn)
    {
        ...
        if (debug)
            debugEV
                    << "I (" << nodeAddr << ") received a message from node " << m->getSrcAddr() << " of size "
                            << m->getBitLength() << "." << endl;
        nbPacketsReceived++;
    ...
    else
    {
        ...
        if (debug)
        {
            ...
            debugEV
                    << "I (" << nodeAddr << ") received a message from node "
                            << (static_cast<ApplPkt*>(msg))->getSrcAddr() << " of size " << m->getBitLength() << "."
                            << endl;
        }
        delete msg;
    }
