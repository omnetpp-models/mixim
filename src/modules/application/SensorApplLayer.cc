...
{
    if (nbPackets > sentPackets && trafficType != 0)
    { // We must generate packets
        ...
        switch (trafficType)
        {
            ...
            case UNKNOWN:
            default:
                EV << "Cannot generate requested traffic type (unimplemented or unknown)." << endl;
    ...
    switch (msg->getKind())
    {
        case DATA_MESSAGE:
            ...
            if (stats)
            {
                ...
                {
                    debugEV
                            << "Received a data packet from host[" << m->getSrcAddr() << "], latency=" << theLatency
                                    << ", collected " << hostsLatency(m->getSrcAddr()).getCount() << "mean is now: "
