...
class MIXIM_API TestApplication : public BaseModule
{
    ...
    public:
        TestApplication() :
                BaseModule(), dataOut(-1), dataIn(-1), ctrlOut(-1), ctrlIn(-1), delayTimer(NULL), nbPackets(0), remainingPackets(
                        0), headerLength(0), nodeAddr(), dstAddr(), trafficParam(0.0), debug(false), stats(false), trace(
                        false), flood(false), isTransmitting(false), INITIAL_DELAY(0), PAYLOAD_SIZE(0), nbPacketsReceived(
                        0), latencies(), latenciesRaw(), testStat()
        {
        }
