...
Decider80211::Decider80211(DeciderToPhyInterface* phy, double threshold, double sensitivity, int channel, int myIndex,
        bool debug) :
        BaseDecider(phy, sensitivity, myIndex, debug), snrThreshold(threshold), centerFrequency(0)
{
    assert(1 <= channel);
    assert(channel <= 14);
...
double Decider80211::calcChannelSenseRSSI(simtime_t_cref start, simtime_t_cref end) const
{
    ...
    Mapping::argument_value_t rssi = MappingUtils::findMax(*rssiMap, min, max,
...
DeciderResult* Decider80211::checkIfSignalOk(MiximAirFrame* frame)
{
    ...
    Mapping::argument_value_t snirMin = MappingUtils::findMin(*snrMap, min, max,
            Argument::MappedZero /* the value if no minimum will be found */);
