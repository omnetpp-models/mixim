...
class MIXIM_API PhyLayerBattery : public PhyLayer
{
    protected:
        ...
        enum Activities
        {
            SLEEP_ACCT = 0, RX_ACCT, TX_ACCT, SWITCHING_ACCT, DECIDER_ACCT,
        };
    ...
    public:
        PhyLayerBattery() :
                PhyLayer(), numActivities(0), sleepCurrent(0), rxCurrent(0), decodingCurrentDelta(0), txCurrent(0), setupRxCurrent(
                        0), setupTxCurrent(0), rxTxCurrent(0), txRxCurrent(0)
        {
        }
