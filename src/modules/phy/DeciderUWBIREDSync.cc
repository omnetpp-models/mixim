...
DeciderUWBIREDSync::DeciderUWBIREDSync(DeciderToPhyInterface* iface, PhyLayerUWBIR* _uwbiface, double _syncThreshold,
        ...
        DeciderUWBIRED(iface, _uwbiface, _syncThreshold, _syncAlwaysSucceeds, _stats, _trace,
                alwaysFailOnDataInterference), tmin(_tmin), syncVector(), argSync()
{
...
bool DeciderUWBIREDSync::attemptSync(Signal* s)
{
    ...
    while (search
            && !(aSignal.getReceptionStart() == s->getReceptionStart() && aSignal.getDuration() == s->getDuration()))
    {
    ...
    if (search && it != syncVector.end())
    {
        ...
        if (nextSignal.getReceptionStart() < aSignal.getReceptionEnd() + tmin)
        {
