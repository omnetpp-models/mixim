...
simtime_t SNRThresholdDecider::canAnswerCSR(const CSRInfo& requestInfo)
{
    ...
    assert(request->getSenseMode() == UNTIL_IDLE || request->getSenseMode() == UNTIL_BUSY);
...
simtime_t SNRThresholdDecider::processSignalEnd(AirFrame* frame)
{
    ...
    bool aboveThreshold = checkIfAboveThreshold(snrMap, MappingUtils::post(start), MappingUtils::pre(end));
