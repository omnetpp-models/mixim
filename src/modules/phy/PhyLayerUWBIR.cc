...
AnalogueModel* PhyLayerUWBIR::createUWBIRStochasticPathlossModel(ParameterMap & params) const
{
    ...
    if (it == params.end())
    {
        error("Could not find required double parameter <mu_gamma> in the Ghassemzadeh channel xml description file.");
    }
    ...
    if (it == params.end())
    {
        error("Could not find required double parameter <mu_sigma> in the Ghassemzadeh channel xml description file.");
    }
    ...
    if (it == params.end())
    {
        error("Could not find required bool parameter <isEnabled> in the Ghassemzadeh channel xml description file.");
    }
    ...
    return new UWBIRStochasticPathlossModel(PL0, mu_gamma, sigma_gamma, mu_sigma, sigma_sigma, isEnabled, shadowing);
}
...
Decider* PhyLayerUWBIR::getDeciderFromName(std::string name, ParameterMap& params)
{
    ...
    if (it == params.end())
    {
        error("Could not find required double parameter <syncThreshold> in the decider xml configuration file.");
    }
    ...
    if (it == params.end())
    {
        error("Could not find required boolean parameter <stats> in the decider xml configuration file.");
    }
    ...
    if (it == params.end())
    {
        error("Could not find required boolean parameter <trace> in the decider xml configuration file.");
    }
    ...
    if (it == params.end())
    {
        error("Could not find required boolean parameter <syncAlwaysSucceeds> in the decider xml configuration file.");
    }
    ...
    if (name == "DeciderUWBIREDSyncOnAddress")
    {
        ...
        if (it == params.end())
        {
            error("Could not find required int parameter <addr> in the decider xml configuration file.");
        }
        addr = LAddress::L2Type(it->second.longValue());
        uwbdecider = new DeciderUWBIREDSyncOnAddress(this, this, syncThreshold, syncAlwaysSucceeds, stats, trace, addr,
    ...
    if (name == "DeciderUWBIREDSync")
    {
        ...
        if (it == params.end())
        {
            error("Could not find required double parameter <syncMinDuration> in the decider xml configuration file.");
        }
        tmin = it->second.doubleValue();
        uwbdecider = new DeciderUWBIREDSync(this, this, syncThreshold, syncAlwaysSucceeds, stats, trace, tmin,
                alwaysFailOnDataInterference);
    }

    if (name == "DeciderUWBIRED")
    {
        uwbdecider = new DeciderUWBIRED(this, this, syncThreshold, syncAlwaysSucceeds, stats, trace,
                alwaysFailOnDataInterference);
    }
    return uwbdecider;
}
