...
AnalogueModel* PhyLayer::initializeBreakpointPathlossModel(ParameterMap& params) const
{
    ...
    else // carrierFrequency has not been specified in config.xml
    {
        if (cc->hasPar("carrierFrequency")) // parameter carrierFrequency has been specified in ConnectionManager
        {
            ...
            carrierFrequency = cc->par("carrierFrequency").doubleValue();
            coreEV
                    << "createPathLossModel(): carrierFrequency set from ConnectionManager to " << carrierFrequency
                            << endl;
        }
...
Decider* PhyLayer::initializeDecider80211(ParameterMap& params)
{
    ...
    return new Decider80211(this, threshold, sensitivity, radio->getCurrentChannel(), findHost()->getIndex(), coreDebug);
}
...
Decider* PhyLayer::initializeSNRThresholdDecider(ParameterMap& params)
{
    ...
    {
        ev << "No busy threshold defined for SNRThresholdDecider. Using" << " phy layers sensitivity as busy threshold."
                << endl;
    }
    else
    {
    ...
    return new SNRThresholdDecider(this, snrThreshold, sensitivity, busyThreshold, findHost()->getIndex(), coreDebug);
}
